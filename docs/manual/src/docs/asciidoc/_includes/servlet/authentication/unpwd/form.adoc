...
* The `AuthenticationSuccessHandler` is invoked. Typically this is a `SavedRequestAwareAuthenticationSuccessHandler` which will redirect to a request saved by <<servlet-exceptiontranslationfilter,`ExceptionTranslationFilter`>> when we redirect to the log in page.
